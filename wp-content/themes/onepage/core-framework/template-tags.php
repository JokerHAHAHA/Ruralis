...
function igthemes_entry_footer() {
    ...
    if ( 'post' == get_post_type() ) {
        ...
        if ( $categories_list && igthemes_categorized_blog() ) {
            printf( '<span class="cat-links">' . esc_html__( '%1$s', 'onepage' ) . '</span>', $categories_list );
        }

        /* translators: used between list items, there is a space after the comma */
        $tags_list = get_the_tag_list( '', esc_html__( ', ', 'onepage' ) );
        if ( $tags_list ) {
            printf( '<span class="tags-links">' . esc_html__( '%1$s', 'onepage' ) . '</span>', $tags_list );
        }
    }

    if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
        echo '<span class="comments-link">';
        comments_popup_link( esc_html__( 'Leave a comment', 'onepage' ), __( '1 Comment', 'onepage' ), esc_html__( '% Comments', 'onepage' ) );
        echo '</span>';
    }
